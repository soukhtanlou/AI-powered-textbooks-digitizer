...
const App: React.FC = () => {
  ...
  const handlePageAnalysis = async () => {
      ...
      try {
          ...
          updateCurrentPage({ 
              ...
              isPageAnalysisConfirmed: false // Reset confirmation on new analysis
          });
          updateState({ isLoading: false });
  ...
  const generateTeacherContent = async () => {
      ...
      if (!p.isPageAnalysisConfirmed) {
          if(!confirm("تحلیل این صفحه هنوز تایید نشده است. هوش مصنوعی ممکن است بر اساس اطلاعات غلط تدریس کند. ادامه می‌دهید؟")) return;
      }

      updateState({ isLoading: true });
      try {
          const b64 = await getCurrentRawBase64();
          // Pass the verified text and description as ground truth
          const script = await GeminiService.generateTeacherScript(
              b64, 
              c.context, 
              p.aiAnalysis,
              p.extractedText, // Ground Truth
              p.imageDescription // Ground Truth
          );
  ...
  const generateStoryboardPrompt = async () => {
      ...
      if (!p) return;
      if (!p.isPageAnalysisConfirmed) {
         if(!confirm("تحلیل صفحه تایید نشده. ممکن است تصویرسازی اشتباه شود. ادامه؟")) return;
      }
      updateState({ isLoading: true });
      try {
          // Changed: Always generate new prompt (removed `if (!p.storyboardPrompt)` check)
          const b64 = await getCurrentRawBase64();
  ...
  const executeStoryboardGen = async () => {
      ...
      try {
          ...
          const assetId = `${p.id}_storyboard`;
          await saveAsset(assetId, blob);
          ...
          <div className="h-full bg-slate-100 flex items-center justify-center p-4">
              <div className="w-full max-w-md bg-white h-full rounded-2xl shadow-2xl overflow-hidden flex flex-col relative border border-slate-300">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-6">
                      ...
                      {/* NOTE: We removed the text display here as requested by user. Only multimedia content is shown in player. */}
